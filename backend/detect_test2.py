...
# Hệ số quy đổi PCU theo báo cáo
PCU_FACTORS = {
    0: 1.0,   # Car
    1: 2.5,   # Bus
    2: 2.0,   # Truck
...
C_MAX = 60  # Chu kỳ tối đa (giây) – dùng trong công thức T_green
...
try:
    srPort = serial.Serial(UART_PORT, BAUD, timeout=1)
    time.sleep(2)  # đợi ESP32 ổn định sau khi mở cổng
...
def transmit_data_to_mcu(main_green_time, cross_green_time):
    """Gửi thời gian đèn xanh xuống ESP32 theo protocol đã thử thành công"""
    packet = f"M:{main_green_time}|C:{cross_green_time}\n"
    print(f"[UART TX] Transmitting to MCU: {packet.strip()}")

    global srPort
    if srPort is None:
        print("Error: Serial port not initialized. Cannot transmit data.")
        return False
    else:
        try:
...
def calculate_pcu(counts_dict):
    """Tính tổng PCU từ dictionary counts {0: n_car, 1: n_bus, ...}"""
    total = 0.0
...
def calculate_t_green(q_main, q_cross, c_max=C_MAX):
    """
    Tính thời gian đèn xanh theo công thức báo cáo.
    Có ràng buộc Clamp [15, 45] và Fallback khi Total_PCU = 0.
    """
    total = q_main + q_cross
    if total == 0:
        # Fallback / Night Mode: chia đều 30s/30s
        return 30.0, 30.0

    t_main = (q_main / total) * c_max
    t_cross = c_max - t_main

    # Clamp Function: 15 ≤ T_green ≤ 45
...
def print_traffic_stats(cam_id, config):
    """In ra Terminal theo format báo cáo (Hình 3.1)"""
    ...
    print("=" * 55)
    print()
    # ===== GỬI XUỐNG ESP32 (protocol cũ đã thử thành công) =====
    # Gửi float 1 chữ số thập phân. Nếu ESP32 chỉ nhận int thì đổi thành:
...
def generate_frames(cam_id: str):
    ...
    # In thống kê định kỳ (mỗi N frame) để không spam Terminal
    ...
    while cap.isOpened():
        ...
        if not success:
            # Khi video loop lại → in thống kê cuối chu kỳ rồi reset
            print_traffic_stats(cam_id, config)
            cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
            track_history.clear() 
            counted_ids.clear()
            # Reset counters nếu muốn đếm lại từ đầu mỗi vòng video
            # (giữ nguyên nếu muốn tích lũy – hiện tại reset cho khớp demo)
        ...
        results = local_model.track(frame, persist=True, conf=0.25, imgsz=640, tracker="bytetrack.yaml", verbose=False)
                        ...
                        if unique_id not in counted_ids:
                            ...
                            for line in config["main_lines"]:
                                if intersect(prev_pt, curr_pt, line[0], line[1]):
                                    ...
                                    # ĐÃ BỔ SUNG LẠI LOGIC GHI EVENT TRỤC CHÍNH
                            ...
                            for line in config["cross_lines"]:
                                if intersect(prev_pt, curr_pt, line[0], line[1]):
                                    ...
                                    # ĐÃ BỔ SUNG LẠI LOGIC GHI EVENT TRỤC PHỤ
                                    now = datetime.now().strftime("%H:%M:%S")
                                    event = {
                                        "id": int(time.time() * 1000) + obj_id,
                                        "time": now,
                            ...
                            # Giới hạn mảng lưu trữ 15 sự kiện gần nhất
                            if len(config["events"]) > 15:
                                config["events"].pop()

                    track_history[unique_id] = curr_pt

        # In thống kê định kỳ ra Terminal (không spam mỗi frame)
...
if __name__ == "__main__":
    ...
    # Chọn ngã tư muốn test: "1", "2" hoặc "3"
    ...
    try:
        for _ in generate_frames(cam_id):
            # Generator đã tự in PCU + T_green mỗi 5 giây
            # và mỗi khi video loop lại
            pass
    except KeyboardInterrupt:
        print("\n[*] Đã dừng bởi người dùng.")
        # In lần cuối trước khi thoát
        config = INTERSECTIONS.get(cam_id)
        if config:
            print_traffic_stats(cam_id, config)
